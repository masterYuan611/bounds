...
import { getBondBestPage, exportBestPage } from '@/api/optimalBonds'
...
export default {
  ...
  methods: {
    ...
    getTopData() {
      ...
      getBondBestPage(this.topParams)
        .then(({ data }) => {
    ...
    // 滚动分页
    handleScroll({ scrollTop: top, isX, $event }) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (flag === 'insert') {
          if (isMeetFilter) {
            ...
            this.insertNum++
            if (map.is_hot === '0') continue // 新增报价时，如果该条债券为热点债券，同时也需要对hotList进行新增
          ...
          if (isMeetFilter) {
            ...
            if (map.is_hot === '0') continue // 报价更新时，如果该条债券为热点债券，同时也需要对hotList进行更新
            if (hotIndex === -1) {
              this.hotList = [{ ...map }, ...this.hotList]
              //   this.setStatus(this.hotList[0])
            } else {
    ...
    handleSocketHotData(list) {
      for (let i = 0; i < list.length; i++) {
        ...
        if (isHot === '1') {
          ...
          this.setStatus(this.topData[index])
          const hotIndex = this.hotList.findIndex((item) => item.id === fineId)
          if (hotIndex > -1) {
            this.hotList.splice(hotIndex, 1)
          }
        }
